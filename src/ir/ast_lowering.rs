...
// impl<'a> IrCtx<'a> {
//     pub fn lower_ast(&self, ast: AST<'a>) -> IR<'a> {
//         let module = ast.0;

//         let stmts: Vec<&mut Stmt<'a>> = module
//             .stmts
//             .into_iter()
...
//     fn lower_stmt(&self, stmt: crate::ast::node::statement::Stmt<'a>) -> &mut Stmt<'a> {
...
//                 let mut ir_args: Vec<&mut FunctionArg<'a>> =
...
//                 let mut ir_attrs: Vec<&mut StructAttr<'a>> =
...
//     fn lower_expr(&self, expr: crate::ast::node::expression::Expr<'a>) -> &mut Expr<'a> {
//         let _ir_expr: Expr<'a> = match expr.value {
